...
<code>Ecommerce::Merchants::SecureTrading::Payment.create(customer_info, card, order, amount, settlement_day = 1)</code>

*Schedule a repeat payment* that uses continuous auth:

<code>Ecommerce::Merchants::SecureTrading::Payment.repeat(customer_xml, order_object, amount)</code>

*Reverse authorisation* (allow the customer to cancel an order before payment is authorised):

<code>Ecommerce::Merchants::SecureTrading::Payment.auth_reversal(customer_xml, order, amount)</code>

*Check the status of settlement*:

<code>Ecommerce::Merchants::SecureTrading::Payment.settlement_query</code>
